/**
 * PASSO 05 — HISTÓRICO COM FlatList
 * ─────────────────────────────────────────────────────────────
 * Toda tentativa fica registrada num histórico que aparece abaixo
 * do jogo, renderizado com FlatList — exatamente o componente
 * estudado na AULA 05.
 *
 * O que mudou em relação ao Passo 04:
 *   + Estado novo: historico (array de objetos)
 *   + Cada item: { id, palpite, resultado, distancia, cor }
 *   + setHistorico([novoItem, ...historico]) — adiciona no topo
 *   + Componente FlatList listando o histórico
 *   + Layout reorganizado: jogo no topo, histórico abaixo
 *
 * Conceitos novos:
 *   • FlatList com data, keyExtractor e renderItem
 *   • Padrão imutável: NÃO usar historico.push() — sempre criar
 *     novo array com spread [...arr] ou [novo, ...arr]
 *   • Componente extraído (ItemHistorico) para legibilidade
 *   • Date.now() como id único simples
 *
 * Atenção:
...
// Componente filho — boa prática para itens de lista
function ItemHistorico({ item }) {
  return (
    <View style={styles.itemHistorico}>
      <Text style={[styles.itemPalpite, { color: item.cor }]}>
        {item.palpite}
      </Text>
      <Text style={styles.itemTexto}>
        {item.resultado === 'acertou'
          ? '✓ Acertou'
          : `${item.label} — ${item.direcao}`}
      </Text>
    </View>
  );
}
...
export default function App() {
  ...
  function iniciarJogo(nivel) {
    ...
    setGanhou(false);
    setPerdeu(false);
    setHistorico([]); // ← limpa histórico ao reiniciar
  }
  ...
  function tentar() {
    ...
    // PADRÃO IMUTÁVEL: cria novo array, NÃO modifica o atual
